use std::{collections, env, num, rc, time};
    ...
    T: Application,
{
    fn resumed(&mut self, event_loop: &event_loop::ActiveEventLoop) {
        ...
        let config = T::config();
        let (pwidth, pheight) = config.physical_size();
            ...
            event_loop
                .create_window(
                    window::WindowAttributes::default()
                        ...
                        .with_inner_size(dpi::PhysicalSize::new(pwidth as u32, pheight as u32))
                        .with_resizable(false),
                )
                .unwrap(),
        );
        window.set_cursor_grab(window::CursorGrabMode::Locked).unwrap();
        window.set_cursor_visible(false);

        let context = softbuffer::Context::new(rc::Rc::clone(&window)).unwrap();
        let back_buffer = render::RenderTarget::new([config.width, config.height]);
        let mut surface = softbuffer::Surface::new(&context, rc::Rc::clone(&window)).unwrap();
        surface
            .resize(
                num::NonZeroU32::new(pwidth as u32).unwrap(),
                num::NonZeroU32::new(pheight as u32).unwrap(),
            )
            .unwrap();
    ...
    fn window_event(
        ...
        match event {
            ...
            | event::WindowEvent::RedrawRequested => {
                ...
                state.inner_state.frame(&mut context, &mut state.back_buffer);

                let scale = state.config.scale;
                let rw = state.config.width;
                let rh = state.config.height;
                let pw = rw * scale;
                let mut buffer = state.surface.buffer_mut().unwrap();
                for ry in 0..rh {
                    for rx in 0..rw {
                        let color = state.back_buffer[ry * rw + rx];
                        let block_origin = ry * scale * pw + rx * scale;
                        for dy in 0..scale {
                            let row = block_origin + dy * pw;
                            for dx in 0..scale {
                                buffer[row + dx] = color;
                            }
                        }
